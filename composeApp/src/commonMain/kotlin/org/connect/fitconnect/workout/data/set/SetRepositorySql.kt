...
import org.connect.fitconnect.domain.workout.toSetGroupedByWorkoutDate
...
class SetRepositorySql(
    ...
    override fun getAllSetsGroupedByDate(): Flow<Result<List<SetGroupedByWorkoutDate>, DataError>>
    = flow {
        setDataSource.getSetsGroupedByDate()
            .catch {
                emit(Result.Error(DataError.UNKNOWN))
            }
            .collect{ dtos ->
                emit(Result.Success(dtos.map { dto -> dto.toSetGroupedByWorkoutDate() }))
            }
    }
    ...
    override fun getAllSetsForExercise(exerciseId: Int): Flow<Result<List<Set>, DataError>> = flow{
        setDataSource.selectSetsByExerciseId(exerciseId.toLong())
            .catch {
                emit(Result.Error(DataError.UNKNOWN))
            }
            .collect { dtos ->
